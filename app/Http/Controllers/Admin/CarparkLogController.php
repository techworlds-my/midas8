...
class CarparkLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('location.is_enable', function ($row) {
                return $row->location ? (is_string($row->location) ? $row->location : $row->location->is_enable) : '';
            });
    ...
    public function create()
    {
        ...
        $carplates = VehicleMangement::all()->pluck('car_plate', 'id')->prepend(trans('global.pleaseSelect'), '');
